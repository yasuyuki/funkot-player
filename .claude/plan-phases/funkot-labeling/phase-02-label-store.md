...
`store.rs:363` の `funkot: Option<bool>` は**「解析を上書きする」**意味で、
...
こうすると既存の読み取り経路 — `effective_is_funkot`（`store.rs:370`）、
`gated_non_funkot`（`lib.rs:4362`）、`track_row`（`lib.rs:4406`）、既存テスト
（`store.rs:2002`、`lib.rs:4598`）— が**一切変更なしで動く**。
...
- `LABELS_FILE = "labels.json"` を定数一覧（`store.rs:45-52`）に追加
- `TrackLabel { verdict: bool, labeled_at_ms: u64 }`、
  `Labels = BTreeMap<String, TrackLabel>`（キーは content hash）
- `load_labels` / `save_labels` — `load_flags` / `save_flags`（`store.rs:656-678`）を
...
- `TrackRow`（`lib.rs:4310`）に `label: Option<bool>` を追加
- `generate_handler!`（`lib.rs:5779`）に3本を登録

見本にする既存実装: `set_bars_impl`（`lib.rs:5047-5116`）と
`dismiss_flags`（`lib.rs:3894-3922`）。どちらも `SAVE_LOCK` を取り、
更新後の値を返して UI が即反映できる形。

### `src/lib/tauri.ts`

- `TrackRow` interface に `label` を**同名で**追加。
  `tauri.ts:1-8` に「serde の snake_case をそのままミラーせよ、ズレると
...
- content hash がキー。パス変更・リネームに耐える（`store.rs:17-18`）
- ユーザー所有データなので**解析キャッシュには置かない**（`store.rs:20-32`）。
  `data_dir` 直下
- `save_labels` の失敗は warn のみで致命にしない（`lib.rs:5109-5113` に倣う）
- **既存テストを1件も壊さない。** 特に `gated_non_funkot_skips_only_analysed_non_funkot`
  （`lib.rs:4598`）と `bar_override_funkot_round_trip`（`store.rs:2002`）
...
- ラベル対象の同定は content hash 経由。`store::resolve_content_hash` と
  `hash-index.json`（`store.rs:412-462`）が path→hash を橋渡しする
- `SAVE_LOCK` のロック順序（`lib.rs:2400-2420` のコメント: `SAVE_LOCK` → queue →
  render）を守る
