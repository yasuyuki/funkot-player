...
現状 `classify_is_funkot`（`funkot-core/src/analysis.rs`）は
`GridLock::verdict`（`funkot-core/src/analysis.rs`）で3条件を見て **bool しか返さない**。
`classify_scores`（`funkot-core/src/lib.rs`） 『the scores themselves are not stored』と明記済み。
スコアを見る唯一の手段は `probe_classification`（`funkot-core/src/analysis.rs`）→
`examples/classify_probe` で、これは **`analyze` とは別経路で音声を再デコード・
再解析する**（`ClassifyProbe`（`funkot-core/src/analysis.rs`））。798曲では非現実的。

## 対象範囲

- `funkot-core/src/analysis.rs` — `classify_is_funkot` が判定 + head/tail 各3値を返す。
  測定は `SideLock` / `measure`（`funkot-core/src/analysis.rs`）が**既に計算済み**なので新規計算は無い
...
- `CACHE_VERSION`（`funkot-core/src/cache.rs`） — `CACHE_VERSION` 13 → 14。`CACHE_VERSION`（`funkot-core/src/cache.rs`）の bump コメントの慣習に従い
  bump 理由を1行残す
...
- しきい値（`CLASSIFY_MIN_Z` 8.5 / `CLASSIFY_MIN_Z_RATIO` 0.75 /
  `CLASSIFY_MAX_HALF_RATIO` 1.40、`CLASSIFY_MIN_Z` / `CLASSIFY_MIN_Z_RATIO` / `CLASSIFY_MAX_HALF_RATIO`（`funkot-core/src/analysis.rs`））の値は**変更しない**。
  調整は 06 で正解データが揃ってから
...
- `CACHE_VERSION` bump は解析キャッシュを**全破棄**する（`load`（`funkot-core/src/cache.rs`）は
  version 不一致で `None` を返す）。手修正した小節数は `*_manual` +
  `reapply_overrides`（`src-tauri/src/lib.rs`）で生き残る設計だが、
  **bump 前に現物のキャッシュディレクトリを確認すること**
- `classify_is_funkot` は private 関数で `analyze`（`funkot-core/src/analysis.rs`）からのみ
  呼ばれる。呼び出し側の変更は1箇所で済むはず

## 受け入れ条件

1. `cargo test -p funkot-core` が通る
2. `CACHE_VERSION == 14`、`CACHE_VERSION`（`funkot-core/src/cache.rs`）に bump 理由の行が増えている
...
- この計画の完了後、**798曲の全再解析が必要になる**。player を起動して
  `spawn_analysis_worker`（`src-tauri/src/lib.rs`）、**単一スレッド直列**に流させる。
