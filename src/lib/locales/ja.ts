...
export const ja: Messages = {
  ...
  playlistNormal: "再生待ちリスト",
  playlistLoading: "プレイリストを読込中…",
  playlistTrackCount: (count) => `${count}曲`,
  playlistDestination: (name) => `追加先：${name}`,
  playlistAddLabel: (title, destination) => `${title} を ${destination} に追加`,
  playlistAddResult: (summary, destination) => `${destination}へ：${summary}`,
  playlistChoose: "次に再生するリストを選択",
  playlistSearch: "プレイリストを検索",
  playlistNewName: "新しいプレイリスト名",
  playlistCreateUse: "作成して使う",
  playlistCount: (remaining, total) => `残り${remaining} / 全${total}曲`,
  playlistMenu: "プレイリスト操作",
  playlistEditAll: "全曲を表示・編集",
  playlistShowRemaining: "残りの曲を表示",
  playlistRestart: "先頭からやり直す（再生中の曲は続行）",
  playlistEnded: "再生終了",
  ...
  playlistDuplicate: "複製",
  playlistDelete: "削除",
  playlistSaveQueue: "再生待ちをプレイリストとして保存",
  playlistSaveQueueScope: "表示中の次に再生する曲を、準備済み・自動選曲も含めてコピーします。現在の曲は含めず、通常キューと再生元は変えません。",
  playlistSave: "保存",
  playlistConfirmName: (name) => `削除するには「${name}」と入力`,
  playlistReadOnly: "保存データを読み込めないため、プレイリストは読み取り専用です。通常キューは再生できます。",
  playlistSaveFailedRetry: "直前のプレイリスト変更を保存できませんでした。保存先を確認してやり直してください。",
  playlistSkipped: (count) => `${count}曲をスキップ`,
  playlistStatus: (status) => ({ pending: "次に再生", preparing: "準備中", prepared: "準備済み", missing: "見つからない", unplayable: "再生できない", played: "再生済み", current: "再生中" })[status] ?? status,
  playlistFailureReason: (reason) => ({ missing: "ファイルが見つかりません。", non_funkot: "Funkotのみの設定で除外されました。", identity_changed: "追加後にファイルが変わりました。", identity_unavailable: "この曲を識別できません。", load_failed: "曲を読み込めませんでした。", loader_failure: "曲を読み込めませんでした。" })[reason] ?? "曲を読み込めませんでした。",
  playlistCreateSavedSelectionFailed: (name, reason) => `「${name}」を作成しましたが、切替できませんでした：${reason}`,
